...
def _ais_stream_loop():
    ...
    proxy_script = os.path.join(
        os.path.dirname(os.path.dirname(__file__)), "ais_proxy.js"
    )
                ...
                if msg_count % 5000 == 0:
                    with _vessels_lock:
                        ...
                        stale = [
                            ...
                            if v.get("_updated", 0) < prune_cutoff
                        ]
                        for k in stale:
                            del _vessels[k]
                        count = len(_vessels)
...
def get_vessels_by_mmsi_list(mmsi_list: list[int]) -> dict[int, dict]:
    """Return a snapshot of specific MMSIs for carrier watchlist checks."""
    with _vessels_lock:
        return {mmsi: dict(v) for mmsi, v in _vessels.items() if mmsi in mmsi_list}
